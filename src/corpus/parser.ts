...
export function chunkMarkdown(
  ...
  // Article pattern: Artículo/Articulo followed by number, ordinal, cardinal, or "único", with optional "bis"
  // Matches: "Artículo 1", "Articulo 1", "Artículo 1º", "Artículo 2ª", "Artículo único", "Artículo 10 bis"
  const articlePattern = /^Art[íi]culo\s+(\d+[ººªª]?\s*(?:bis)?|(?:[uú]nico))/i;

  for (const line of lines) {
    const trimmedLine = line.trim();
    
    // Check for article pattern first (with or without heading markers)
    const articleWithHeadingMatch = trimmedLine.match(/^#{1,6}\s+Art[íi]culo\s+(\d+[ººªª]?\s*(?:bis)?|(?:[uú]nico))/i);
